...
SELECT CONCAT('user', LPAD(n, 6, '0')), CONCAT('user', LPAD(n, 6, '0'), '@wiw.com'),
...
FROM users u WHERE u.username LIKE 'user%';
COMMIT;
SELECT '2. UserProfile 생성 완료' AS status, COUNT(*) AS count FROM user_profile;

-- 3. BucketItem (10만 개씩 10번 = 100만 개)
-- 아래 쿼리를 10번 실행 (시작/종료 번호만 변경)
INSERT INTO bucket_item (profile_id, category, content, status, due_date, cheer_count, comment_count, created_at, updated_at)
WITH RECURSIVE numbers AS (
    SELECT 1 AS n UNION ALL SELECT n + 1 FROM numbers WHERE n < 100000
)
SELECT 1 + MOD(n - 1, 100000),
       ...
       DATE_ADD(NOW(), INTERVAL FLOOR(RAND() * 365) DAY), 0, 0,
       DATE_SUB(NOW(), INTERVAL FLOOR(RAND() * 180) DAY), NOW()
FROM numbers;
COMMIT;
SELECT '3. BucketItem 1차 완료' AS status;

-- 위 쿼리를 9번 더 실행 (numbers 범위만 변경: 100001~200000, 200001~300000, ...)

-- 4. Comment (간소화 버전 - BucketItem 10만 개씩 처리)
-- 생략 가능 (시간이 오래 걸림)
